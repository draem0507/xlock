...
public class ZkPrimitive {
     ...
     */
    private static final byte[] emptyNode = new byte[] {};
     ...
     * <p>
     * Setting this to {@code true} will cause some subclasses to cancel their activities, on the basis that they are no longer able to complete a
     * given task.
     */
    ...
    /**
     * A signalling watcher, whose job it is to call {@link Condition#signal()} or {@link Condition#signalAll()} to notify any threads sleeping
     * through the
     ...
     *
     * @param baseNode
     *            the base node to use
     * @param zkSessionManager
     *            the session manager to use
     * @param privileges
     *            the privileges for this node.
     */
     ...
     * <p>
     * Note: This method does NOT create elements recursively--if the base node is a sub-node of a node which doesn't exist, a NoNode Exception will
     * be thrown.
     *
     * @throws RuntimeException
     *             wrapping a KeeperException if something goes wrong communicating with the ZooKeeper server
    ...
    /**
     * create elements recursively added by jiangxu
     * 
     * @param zooKeeper
     * @param offset
     ...
     */
    private void recursiveCreate(ZooKeeper zooKeeper, int offset) throws KeeperException, InterruptedException {
        offset = StringUtils.indexOf(baseNode, "/", offset);
        if (0 > offset) {
            return;
        }
        String path = StringUtils.substring(baseNode, 0, offset);
        Stat stat = zooKeeper.exists(path, false);

        if (stat == null) {
            zooKeeper.create(path, emptyNode, privileges, CreateMode.PERSISTENT);
        }
        recursiveCreate(zooKeeper, offset + 1);
    }
