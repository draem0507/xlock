 ...
 */
package com.bluedream.distributedlock.engine.zk.lock;

import com.bluedream.distributedlock.common.LockHolder;
import com.bluedream.distributedlock.engine.zk.util.ZkPrimitive;
import com.bluedream.distributedlock.engine.zk.util.ZkSessionManager;
import com.bluedream.distributedlock.engine.zk.util.ZkUtils;
 ...
 * <p>
 * A {@code ReentrantZkLock} is <i>owned</i> by the <i>party</i> (both the current thread and the running ZooKeeper client) which has last
 * successfully locked it, but has not yet unlocked it. Parties invoking {@code lock} will return, successfully acquiring the lock, whenever the lock
 * is not owned by another party. All methods will return immediate success if the current party is the lock owner. This can be checked by
 * {@link #hasLock()}.
 * <p>
 * Note that this implementation is fully <i>fair</i>; multiple concurrent requests by different parties will be granted <i>in order</i>, where the
 * ordering is determined by the ZooKeeper server.
 * <p>
 * This lock supports {@code Integer.MAX_VALUE} repeated locks by the same thread. Attempts to exceed this result in overflows which can create an
 * improper lock state.
 *
 * @author Scott Fines
 * @version 1.0
 ...
 */
public class ReentrantZkLock extends ZkPrimitive implements com.bluedream.distributedlock.lock.Lock {
     ...
     * <p>
     * <p>
     * If the lock is not available, then the current thread becomes disabled for thread scheduling purposes and lies dormant until the lock as been
     * acquired.
     * <p>
     * <p>
     ...
     *                          <ul>
     *                          <li> {@link KeeperException} if the ZooKeeper server encounters a problem <li> {@link InterruptedException} if
     *                          there is a communication problem between the ZooKeeper client and server <li> If the ZooKeeper session expires
     *                          </ul>
     * @inheritDoc
     * @see Lock#lock()
     */
    ...
    public final void lock() {
        ...
        String lockNode;
        try {
            lockNode = zk.create(getBaseLockPath(), super.getEmptyNode(), privileges, CreateMode.EPHEMERAL_SEQUENTIAL);
     ...
     * <p>
     * <p>
     * Note: If the ZooKeeper Session expires while this thread is waiting, an {@link InterruptedException} will be thrown.
     *
     ...
     * @throws RuntimeException wrapping :
     *                          <ul>
     *                          <li>{@link KeeperException} if there is trouble processing a request with the ZooKeeper servers. <li>
     *                          {@link InterruptedException} if there is an error communicating between the ZooKeeper client and servers.
     *                          </ul>
     * @inheritDoc
     */
    @Override
    public final boolean tryLock() {
        ...
        ZooKeeper zk = zkSessionManager.getZooKeeper();
        try {
            String lockNode = zk.create(getBaseLockPath(), super.getEmptyNode(), privileges, CreateMode.EPHEMERAL_SEQUENTIAL);
     ...
     *                              <ol>
     *                              <li value="1">The Thread is interrupted upon entry to the method <li value="2">Another thread interrupts this thread while it is
     *                              waiting to acquire the lock <li value="3">There is a communication problem between the ZooKeeper client and the ZooKeeper server.
     ...
     * <p>
     * Note: This method will return {@code null} <i>unless</i> the current thread is the lock owner. This method is primarily intended to ease the
     * use of shared lock implementations between threads, and should not be used to manage lock state.
     *
     ...
     * @throws RuntimeException wrapping:
     *                          <ul>
     *                          <li> {@link KeeperException} if the ZooKeeper server fails to process the unlock request
     ...
     * <p>
     * When this method has completed, either the state of the ZooKeeper server is such that this party now holds the lock of the correct type, or, if
     * the {@code watch} parameter is true, any and all necessary Watcher elements have been set.
     * <p>
     * Classes which override this method MUST adhere to the requested watch rule, or else the semantics of the lock interface may be broken. That is,
     * if the {@code watch} parameter is true, then a watch needs to have been set by the end of this method.
     * <p>
     * It is recommended that classes which override this method also override {@link #getBaseLockPath()} and {@link #getLockPrefix()} as well.
     *
    ...
    protected boolean tryAcquireDistributed(ZooKeeper zk, String lockNode, boolean watch) throws KeeperException, InterruptedException {
        List<String> lockList = ZkUtils.filterByPrefix(zk.getChildren(baseNode, false), getLockPrefix());
        ZkUtils.sortBySequence(lockList, lockDelimiter);
