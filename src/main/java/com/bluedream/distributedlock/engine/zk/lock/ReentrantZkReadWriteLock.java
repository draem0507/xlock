        ...
        protected boolean tryAcquireDistributed(ZooKeeper zk, String lockNode, boolean watch) throws KeeperException, InterruptedException {
            ...
            if (!writeLocks.isEmpty()) {
                ...
                if ((baseNode + "/" + leadWriteLock).equals(ReentrantZkReadWriteLock.this.writeLock.getLockName())) {
                    zk.create(getBaseLockPath() + lockDelimiter + ZkUtils.parseSequenceString(writeLocks.get(0), lockDelimiter), super.getEmptyNode(), privileges, CreateMode.EPHEMERAL);
            ...
            for (String lock : writeLocks) {
                long lockSeqNbr = ZkUtils.parseSequenceNumber(lock, lockDelimiter);
                if (lockSeqNbr < mySequenceNumber) {
                    //a write lock ahead of us, so add it in
                    aheadWriteLocks.add(lock);
                }
            }
            /*
